...
cheeseTypeBttn.addEventListener('click', function() { kaching.pause(); kaching.currentTime = 0; if (upgrades.cheeseType > cheeseTypes.length - 1) { cheeseTypeBttn.innerHTML = "🧀 HIT BEST CHEESE TYPE: Ricotta 🧀"; cheeseTypeBttn.disabled = true; return; }; money -= upgrades.newCheesePrice; upgrades.pricePerTaco += 0.25; upgrades.newCheesePrice *= 1.5; upgrades.newCheesePrice = fixNumber(upgrades.newCheesePrice, 2); upgrades.cheeseType += 1; kaching.play(); });
...
locationBttn.addEventListener('click', function() {  kaching.pause(); kaching.currentTime = 0; money -= upgrades.locationPrice; upgrades.locationPrice *= 5; upgrades.locations += 1; upgrades.tacosPerClick *= 10 * getRandInt(1,4); kaching.play(); });
